---
name: ship-release
description: Cut a tagged GitHub release from main. Trigger when the user says "ship a release", "cut a release", "tag a release", or invokes /ship-release [size-or-version]. Auto-sizes the semver bump from conventional-commit prefixes in the commit range since the last tag — patch / minor / major — and respects an explicit size hint or exact version. Writes CHANGELOG.md, tags, pushes, creates the GitHub release. Aborts if the tree is dirty, main isn't pushed, or the tag already exists.
---
