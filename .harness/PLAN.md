...
- **Status:** [x] (verified 2026-04-27 on Mac dev box: `shellcheck -x` clean; harness `verify.sh` accepts the file. **Live run on Mac**: claude 2.1.112 (existing), gemini 0.39.0 → 0.39.1 (npm picked up patch), codex 0.125.0 (NEW — `npm install -g @openai/codex` succeeded). Re-run on Mac is idempotent: zshrc PATH markers unchanged, all three CLIs resolved at same versions, npm reports "changed N packages" from dep churn (intrinsic, doesn't affect user-facing binaries — same as v0.0.5 noted in CHANGELOG). **rc_file() dispatch**: `OS=debian SHELL=/bin/zsh` → `~/.zshrc`, `OS=debian SHELL=/bin/bash` → `~/.bashrc`, `OS=macos` → `~/.zshrc` (verified by sourcing the function in three subshells). Debian-only Node-version guard (`check_node_version_debian`) and npm-prefix configuration verified by inspection — actual runtime exercise is `/release`-gate VM work.)
